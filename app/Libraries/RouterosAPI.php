...
{
    // Si se proporciona un puerto válido, se usa ese; de lo contrario, usa el predeterminado
    $this->port = ($port && is_numeric($port) && $port > 0 && $port <= 65535) ? $port : 8728;
    ...
    for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
        ...
        $this->debug("Intento #{$ATTEMPT}: Conectando a {$PROTOCOL}{$ip}:{$this->port}...");
        ...
        if ($this->socket) {
            ...
            if (isset($RESPONSE[0]) && $RESPONSE[0] === '!done') {
                // Verificar si es login post-v6.43 o pre-v6.43
                if (!isset($RESPONSE[1])) {
                    // Post-v6.43: Login exitoso
                    $this->connected = true;
                    break;
                } else {
                    // Pre-v6.43: Autenticación con token
                    $MATCHES = [];
        ...
        // Si no se conectó, esperar antes del próximo intento
        sleep($this->delay);
    }

    if ($this->connected) {
        $this->debug("✅ Conexión exitosa a {$ip}:{$this->port}");
    } else {
        $this->debug("❌ Error: No se pudo conectar a {$ip}:{$this->port}");
    }
